...
import '/screens/chatPart/chatViewPart/chatMessageScreenCtr.dart';
...
class ChatMessageScreen extends StatefulWidget {
  static const screenName = 'ChatMessageScreen';
  ...
  @override
  State createState() => ChatMessageScreenState();
}
///======================================================================================
class ChatMessageScreenState extends StateBase<ChatMessageScreen> {
  StateXController stateController = StateXController();
  ChatMessageScreenCtr controller = ChatMessageScreenCtr();
          ...
          return Builder(
            builder: (context) {
              switch(ctr.mainState){
                ...
                default:
                  if(widget.chatModel.messages.isEmpty){
          ...
          child: StateX(
            ...
            builder: (context, ctr, data) {
              ...
              return ScrollablePositionedList.builder(
                ...
                initialScrollIndex: widget.chatModel.messages.length,
      ...
      child: Row(
        ...
        children: [
          Row(
            ...
            children: [
              CommonRefresh(
                tag: Keys.genDownloadKey_userAvatar(controller.user1.userId),
                builder: (ctx, data){
                  if(!controller.user1.existProfileImageFile){
                    return CircleAvatar(
                      backgroundColor: ColorHelper.textToColor(controller.user1.userName),
                      child: Text(controller.user1.userName.substring(0, 2),
                  ...
                  return GestureDetector(
                    onTap: (){
                      final view = ImageFullScreen(
                          ...
                          heroTag: 'hero${controller.user1.userId}',
                          imageObj: File(controller.user1.genProfilePath()!),
                      );

                      AppNavigator.pushNextPageExtra(context, view, name: ImageFullScreen.screenName);
                    },
                    child: Hero(
                      tag: 'hero${controller.user1.userId}',
                      child: CircleAvatar(
                        backgroundColor: ColorHelper.textToColor(controller.user1.userName),
                        backgroundImage: controller.user1.getProfileProvider(),
                      ),
                    ),
                  );
                },
              ...
              SizedBox(width: 5,),
              Text(controller.user1.userName),
            ],
          ),
          ...
          Row(
            ...
            children: [
              CommonRefresh(
                tag: Keys.genDownloadKey_userAvatar(controller.user2.userId),
                builder: (ctx, data){
                  if(!controller.user2.existProfileImageFile){
                    return CircleAvatar(
                      backgroundColor: ColorHelper.textToColor(controller.user2.userName),
                      child: Text(controller.user2.userName.substring(0, 2),
                        style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold),
                      ),
                    );
                  }
                  ...
                  return GestureDetector(
                    onTap: (){
                      final view = ImageFullScreen(
                        imageType: ImageType.File,
                        heroTag: 'hero${controller.user2.userId}',
                        imageObj: File(controller.user2.genProfilePath()!),
                      );

                      AppNavigator.pushNextPageExtra(context, view, name: ImageFullScreen.screenName);
                    },
                    child: Hero(
                      tag: 'hero${controller.user2.userId}',
                      child: CircleAvatar(
                        backgroundColor: ColorHelper.textToColor(controller.user2.userName),
                        backgroundImage: controller.user2.getProfileProvider(),
                      ),
                    ),
                  );
                },
              ...
              SizedBox(width: 5,),
              Text(controller.user2.userName),
            ],
          ),
        ],
      ),
  ...
  Widget wrapByBubble(ChatMessageModel msg, Widget child){
    final userSender = msg.isLeftSide(widget.chatModel);

    return ChatBubble(
      clipper: ChatBubbleClipper2(type:userSender? BubbleType.receiverBubble: BubbleType.sendBubble),
      backGroundColor: userSender? Colors.white: myColor.withAlpha(80),
      alignment: userSender? Alignment.topLeft : Alignment.topRight,
  ...
  Widget addDateAndState(ChatMessageModel msg, Widget child){
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: msg.isLeftSide(chatModel)? CrossAxisAlignment.end : CrossAxisAlignment.start,
      children: [
        child,
  ...
  Widget genChatView(int idx){
    final msg = widget.chatModel.messages[idx];

    return CommonRefresh(
      key: ValueKey(msg.id),
      tag: Keys.genCommonRefreshTag_chatMessage(msg),
  ...
  Widget getMessageViewByType(ChatMessageModel msg, BuildContext context) {
    final isSender = msg.isLeftSide(widget.chatModel);

    if(msg.type == ChatType.TEXT.typeNum){
      return getTextView(msg);
    }

    final media = ChatManager.getMediaById(msg.mediaId!);
  ...
  Widget getTextView(ChatMessageModel msg){
    return wrapByBubble(
        msg,
  ...
  Widget getDownUploadView(ChatMessageModel msg, ChatMediaModel media){
    if(media.isDraft){
  ...
  Widget getDownloadView(ChatMessageModel msg, ChatMediaModel media){
    return DecoratedBox(
  ...
  Widget getUploadView(ChatMessageModel msg, ChatMediaModel media){
    return DecoratedBox(
      decoration: ShapeDecoration(
        shape: CircleBorder(),
  ...
  Widget getAudioView(ChatMessageModel msg, ChatMediaModel media){
    return wrapByBubble(
      msg,
  ...
  Widget getImageView(ChatMessageModel msg, ChatMediaModel media, bool isSender){
    if(!media.isDownloaded && msg.coverImage == null){
      msg.prepareCover().then((value) {
        CommonRefresh.refresh(Keys.genCommonRefreshTag_chatMessage(msg), null);
      });
    }
  ...
  Widget getVideoView(ChatMessageModel msg, ChatMediaModel media, bool isSender){
  ...
  Widget getFileView(ChatMessageModel msg, ChatMediaModel media, bool isSender){
  ...
  void showPopMenu(ChatMessageModel msg, BuildContext context){
    final box = WidgetHelper.getRenderBox(context);
    ChatMediaModel? media;

    if(msg.mediaId != null) {
      media = ChatManager.getMediaById(msg.mediaId!);
    }
