...
class CManagementWithoutSetScreenCtr implements ViewController {
  late CManagementWithoutSetScreenState state;
  late Requester commonRequester;
  UserModel? user;
  late TextEditingController searchEditController;
  var listChildren = <ItemListViewState>[];
  List<CourseModel> itemList = [];
  CacheMap<String, Uint8List> imageCache = CacheMap(10);
  ...
  void onInitState<E extends State>(E state){
    this.state = state as CManagementWithoutSetScreenState;
  ...
  void tryAgain(State state){
    if(state is CManagementWithoutSetScreenState) {
